...
if __name__ == '__main__':
    ...
    # input_images_dir = "/workspaces/pannellum/examples"
    # output_dir = '/workspaces/pannellum/examples/outputfiles'
    # usage: python 
    input_images_dir = sys.argv[1]
    output_dir = sys.argv[2]


    paths = sorted([*Path(input_images_dir).glob("*.jpg")])
    output_dir = Path(output_dir)
    output_dir.mkdir(exist_ok=True, parents=True)
    
    ncpu = os.cpu_count()    
    args_iterator = zip(paths, [output_dir]*len(paths))
    print("==========================================")
    print(f'Using {ncpu} number of cpus')
    print("==========================================")
    with Pool(5) as p:
        ret_list = p.map(process, args_iterator)